...
mfdb('vignette-tow_data', destroy_schema = TRUE)
...
stopifnot(ut_cmp_equal(unattr(agg_data[[1]]), data.frame(
    year = c('all'),
    step = c('1'),
    area = c('all'),
...
stopifnot(ut_cmp_equal(unattr(agg_data[[1]]), data.frame(
    ...
    mean = c(
        ...
        NULL),
    stringsAsFactors = FALSE), tolerance = 1e-5))
```
