...
ok_group("Aggregates with global taxonomies", local({
    ...
    ok(cmp(where_clause(mdb, c("GEA"), 'tbl.gear_id'),
        "(tbl.gear_id = ANY((SELECT ARRAY(SELECT gear_id FROM gear WHERE name IN ('GEA') OR t_group IN ('GEA')))::INTEGER[]))"))
...
ok_group("Aggregates with CS-specific taxonomies", local({
    ...
    ok(cmp(where_clause(mdb, c("SEA"), 'tbl.sampling_type_id'),
        "(tbl.sampling_type_id = ANY((SELECT ARRAY(SELECT sampling_type_id FROM sampling_type WHERE name IN ('SEA') OR t_group IN ('SEA')))::INTEGER[]))"))
