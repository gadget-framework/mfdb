...
abundance_survey_index_table <- function (mdb, scale_index) {
    if (is.null(scale_index)) {
        ...
        return(c(paste0(
            ...
            " GROUP BY ", paste(seq_len(length(mfdb_measurement_table_defs$survey_index$cols) / 3), collapse = ","),
            ")"), "c.abundance"))
    }
}
...
abundance_core_table <- function (mdb, scale_index) {
    if (is.null(scale_index)) {
        ...
        return(c(paste0(
            ...
            " AND sam.year = si.year",
            " AND sam.month = si.month",
            " GROUP BY ", paste(seq_len(length(mfdb_measurement_table_defs$sample$cols) / 3), collapse = ","),
            ")"), "c.count * c.abundance"))
    }
}
